...
var courseNumbers = []string{"EIEN6001U", "PHIL6301U"}
...
func main() {
	...
	for _, cNo := range courseNumbers {
		...
		for _, course := range courses {
			texts := strings.Split(course.MustText(), "\t")
			selectNum, _ := strconv.Atoi(texts[len(texts)-3])
			fullNum, _ := strconv.Atoi(texts[len(texts)-2])
			if selectNum < fullNum {
				text := fmt.Sprintf("%s的%s有%d空位。", texts[2], texts[1], fullNum-selectNum)
				notify(text)
			}
		}
	}
